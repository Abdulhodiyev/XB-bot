...
from bot.states.ad_states import EditHouseState
from bot.keyboards.inline.house_inline import get_house_edit_kb, get_house_confirm_kb, get_house_admin_kb
from bot.keyboards.reply.common_reply import cancel_kb, done_media_kb
from bot.utils.validators import AutoAdValidator
from bot.utils.formatters import format_house_text, get_admin_footer
...
FIELD_PROMPTS = {
    "rooms": "🔢 <b>Yangi xonalar sonini kiriting:</b>",
    "floor": "🏢 <b>Yangi qavatni kiriting:</b>",
    "area": "📏 <b>Yangi maydonni kiriting:</b>",
    "building_name": "🏪 <b>Yangi obyekt nomini kiriting:</b>",
    "docs": "📄 <b>Hujjatlar holatini kiriting:</b>",
    "conditions": "🚰 <b>Sharoitlarni kiriting:</b>",
    "condition_state": "🛠 <b>Holati/Remontini kiriting:</b>",
    "info": "✍️ <b>Qo'shimcha ma'lumot kiriting:</b>",
...
@router.callback_query(F.data.startswith("e_house_"))
async def ask_new_house_value(call: CallbackQuery, state: FSMContext):
    ...
    if field == "phone" and parts[3] == "numbers":
        field = "phone_numbers"
        ad_id = int(parts[4])
    elif field == "building" and parts[3] == "name":
        field = "building_name"
        ad_id = int(parts[4])
    elif field == "condition" and parts[3] == "state":
    ...
    if field == "photo":
        await state.set_state(EditHouseState.waiting_for_photo)
    ...
    await state.set_state(EditHouseState.waiting_for_text)
...
@router.message(EditHouseState.waiting_for_text)
async def process_edit_house_text(message: Message, state: FSMContext):
    data = await state.get_data()
    field, ad_id, text = data.get("edit_field"), data.get("edit_ad_id"), message.text

    if field != "info" and len(text.strip()) < 1: return await message.answer("❗️ Iltimos, to'liqroq kiriting.")
    if field == "price":
...
@router.message(EditHouseState.waiting_for_photo, F.photo | F.video)
async def accumulate_edit_house_media(message: Message, state: FSMContext):
    data = await state.get_data()
...
@router.message(EditHouseState.waiting_for_photo, F.text == "✅ Rasmlar tayyor")
async def finish_edit_house_media(message: Message, state: FSMContext):
    data = await state.get_data()
